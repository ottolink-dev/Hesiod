...
constexpr const char *A_VREF = "vref";
...
void setup_recast_sag_node(BaseNode &node)
{
  ...
  // attribute(s)
  add_float(node, A_VREF, "vref", 0.5f, 0.f, 1.f);
  add_float(node, A_K, "k", 0.01f, 0.001f, 1.f);
}
    ...
    hmap::for_each_tile(
        ...
        [&node](std::vector<hmap::Array *> p_arrays, const hmap::TileRegion &)
        {
          ...
          *pa_out                       = *pa_in;
          ...
          hmap::recast_sag(*pa_out,
                           node.val<float>(A_VREF),
                           node.val<float>(A_K),
                           pa_mask);
        },
        node.cfg().cm_cpu);
