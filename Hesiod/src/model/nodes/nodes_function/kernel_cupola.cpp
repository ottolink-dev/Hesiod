...
constexpr const char *A_R_CUTOFF  = "r_cutoff";
constexpr const char *A_RADIUS    = "radius";

void setup_kernel_cupola_node(BaseNode &node)
{
  ...
  add_float(node, A_R_CUTOFF, "r_cutoff", 0.5f, 0.f, 1.f);
}

void compute_kernel_cupola_node(BaseNode &node)
{
  ...
  *p_out = hmap::cupola(kernel_shape, node.val<float>(A_R_CUTOFF));
