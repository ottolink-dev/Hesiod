...
void setup_ridge_accentuate_node(BaseNode &node)
{
  ...
  add_float(node, A_STRENGTH, "Strength", 0.02f, 0.f, 0.1f);
...
void compute_ridge_accentuate_node(BaseNode &node)
{
  ...
  hmap::for_each_tile(
      {p_in, p_mask},
      {p_out},
      [strength, ir, reverse](std::vector<const hmap::Array *> p_arrays_in,
                              ...
                              const hmap::TileRegion &)
      {
        ...
        *pa_out                     = *pa_in;
        ...
        hmap::gpu::ridge_accentuate(*pa_out, pa_mask, strength, ir, reverse);
      },
      node.cfg().cm_gpu);
