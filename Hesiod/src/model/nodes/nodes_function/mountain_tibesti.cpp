...
void setup_mountain_tibesti_node(BaseNode &node)
{
  ...
  // attribute(s)
  add_float(node, A_ELEVATION, "elevation", 0.5f, 0.f, 1.f);
  add_float(node, A_SCALE, "scale", 1.f, 0.01f, FLT_MAX);
  add_seed(node, A_SEED, "Seed");
  add_int(node, A_OCTAVES, "Octaves", 8, 0, 32);
  add_float(node, A_PEAK_KW, "peak_kw", 22.f, 0.01f, FLT_MAX);
  add_float(node, A_RUGOSITY, "rugosity", 0.f, 0.f, 1.f);
  add_float(node, A_ANGLE, "angle", 30.f, -180.f, 180.f, "{:.0f}°");
  add_float(node, A_ANGLE_SPREAD_RATIO, "angle_spread_ratio", 0.25f, 0.f, 1.f);
  add_float(node, A_GAMMA, "gamma", 1.5f, 0.01f, 4.f);
  add_bool(node, A_ADD_DEPOSITION, "add_deposition", true);
  add_float(node, A_BULK_AMP, "bulk_amp", 0.5f, 0.f, 2.f);
...
void compute_mountain_tibesti_node(BaseNode &node)
{
  ...
  hmap::for_each_tile(
      ...
      [&node](std::vector<hmap::Array *> p_arrays, const hmap::TileRegion &region)
      {
        ...
        *pa_out = hmap::gpu::mountain_tibesti(region.shape,
                                              node.val<int>(A_SEED),
