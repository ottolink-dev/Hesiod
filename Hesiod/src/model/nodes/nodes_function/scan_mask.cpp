...
constexpr const char *A_BRIGHTNESS = "brightness";
...
void setup_scan_mask_node(BaseNode &node)
{
  ...
  // attribute(s)
  add_float(node, A_CONTRAST, "contrast", 0.5f, 0.f, 1.f);
  add_float(node, A_BRIGHTNESS, "brightness", 0.5f, 0.f, 1.f);
    ...
    hmap::for_each_tile(
        ...
        [&node](std::vector<hmap::Array *> p_arrays, const hmap::TileRegion &)
        {
          ...
          *pa_out              = hmap::scan_mask(*pa_in,
                                    node.val<float>(A_BRIGHTNESS),
                                    node.val<float>(A_CONTRAST));
        },
        node.cfg().cm_cpu);
