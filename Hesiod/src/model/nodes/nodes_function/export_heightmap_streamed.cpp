...
void setup_export_heightmap_streamed_node(BaseNode &node)
{
  ...
  add_filename(node, A_FILENAME, "Filename", "hmap.tif", "Images (*.exr *.tif *.tiff *.btif *.bigtiff)", true);
...
void compute_export_heightmap_streamed_node(BaseNode &node)
{
  ...
  {
    Logger::log()->trace(
        "compute_export_heightmap_streamed_node: [{}]/[{}]: auto export is disabled",
        node.get_node_type(),
        node.get_id());
    return;
  }
  ...
  Logger::log()->trace(
      "compute_export_heightmap_streamed_node: [{}]/[{}]: export path = {}",
      node.get_node_type(),
      node.get_id(),
      export_path.string());
  ...
  {
    Logger::log()->error(
        "compute_export_heightmap_streamed_node: failed to open writer for {}",
        export_path.string());
    return;
  }
