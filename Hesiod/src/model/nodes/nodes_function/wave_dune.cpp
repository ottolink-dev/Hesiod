...
constexpr const char *A_XBOTTOM     = "xbottom";
constexpr const char *A_XTOP        = "xtop";
...
void setup_wave_dune_node(BaseNode &node)
{
  ...
  add_float(node, A_XTOP, "xtop", 0.7f, 0.f, 1.f);
  add_float(node, A_XBOTTOM, "xbottom", 0.9f, 0.f, 1.f);
...
void compute_wave_dune_node(BaseNode &node)
{
  ...
  hmap::for_each_tile(
      ...
      [&](std::vector<hmap::Array *> p_arrays, const hmap::TileRegion &region)
      {
        ...
        *pa_out = hmap::wave_dune(region.shape,
                                  node.val<float>(A_KW),
                                  node.val<float>(A_ANGLE),
                                  node.val<float>(A_XTOP),
                                  node.val<float>(A_XBOTTOM),
                                  phase_rad,
                                  pa_dr,
                                  nullptr,
                                  nullptr,
