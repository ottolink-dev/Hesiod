...
void compute_export_points_to_ply_node(BaseNode &node)
{
  ...
  {
    Logger::log()->trace(
        "compute_export_points_to_ply_node: [{}]/[{}]: auto export is disabled",
        node.get_node_type(),
        node.get_id());
    return;
  }
  ...
  Logger::log()->trace("compute_export_points_to_ply_node: [{}]/[{}]: export path = {}",
                       node.get_node_type(),
                       node.get_id(),
                       export_path.string());
  ...
  auto xr = hmap::rescaled_vector(*p_x, node.val<float>(A_XMIN), node.val<float>(A_XMAX));
  auto yr = hmap::rescaled_vector(*p_y, node.val<float>(A_YMIN), node.val<float>(A_YMAX));
  auto zr = hmap::rescaled_vector(*p_z, node.val<float>(A_ZMIN), node.val<float>(A_ZMAX));
