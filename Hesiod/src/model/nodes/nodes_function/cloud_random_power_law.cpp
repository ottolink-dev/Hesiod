...
void setup_cloud_random_power_law_node(BaseNode &node)
{
  ...
  add_float(node, A_DISTANCE_MAX, "distance_max", 0.15f, 0.001f, 1.f);
  add_float(node, A_ALPHA, "alpha", 0.5f, 0.01f, 4.f);
...
void compute_cloud_random_power_law_node(BaseNode &node)
{
  ...
  *p_out = hmap::random_cloud_distance_power_law(node.val<float>(A_DISTANCE_MIN),
